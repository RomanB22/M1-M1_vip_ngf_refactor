...
def SampleSpikes(spikeTimesList, cfg, preTone=-2., postTone=2, baselinePeriod=(-2., -1.), skipEmpty=False):
    ...
    try:
        baselineStart, baselineEnd = float(baselinePeriod[0]), float(baselinePeriod[1])
    except (TypeError, IndexError, ValueError):
        raise ValueError(
            "baselinePeriod must be a tuple/list with two numeric values: (start_sec, end_sec)."
        )
    if baselineEnd <= baselineStart:
        raise ValueError("baselinePeriod end must be larger than start.")
    ...
    for spkList in spikeTimesList:
        ...
        for spkTimes in spkList:
            # Baseline window from baselinePeriod in seconds
            if (baselineStart <= spkTimes <= baselineEnd):
                # Store as ms, baseline window re-zeroed to 0 at baselineStart
                BaselineTrialsAux.append(1000 * (spkTimes - baselineStart))
    ...
    if cfg.SimulateBaseline is True:
        ...
        sampledSpikesSpan = int(round(1000 * (baselineEnd - baselineStart)))
        if sampledSpikesSpan <= 0:
            raise ValueError("Baseline window must have positive duration (end must be > start).")
...
def _resolve_spiking_data_dir(cwd):
    base_dir = Path(cwd) / "data"
    for data_dir in ("spikingdata", "spikingData"):
        candidate = base_dir / data_dir
        if candidate.exists():
            return candidate
    raise FileNotFoundError(
        f"Could not find spiking data folder in '{base_dir}'. "
        "Expected either 'spikingdata' or 'spikingData'."
    )


def loadThalSpikes(cwd, cfg, skipEmpty=False, baselinePeriod=(-2., -1.)):
    import csv
    import json

    spiking_data_dir = _resolve_spiking_data_dir(cwd)
    thalamic_spikes_file = spiking_data_dir / "thalamic_spikes_by_spkid_tone_onset_(-3.23, 1.44).csv"
    if not thalamic_spikes_file.exists():
        fallback = sorted(spiking_data_dir.glob("thalamic_spikes_by_spkid_tone_onset_*.csv"))
        if not fallback:
            raise FileNotFoundError(
                f"Missing thalamic spikes CSV in '{spiking_data_dir}' "
                "(expected thalamic_spikes_by_spkid_tone_onset_(-3.23, 1.44).csv)."
            )
        thalamic_spikes_file = fallback[0]

    m1_depths_file = spiking_data_dir / "m1_cell_depths_by_session.json"
    if not m1_depths_file.exists():
        no_ext_path = spiking_data_dir / "m1_cell_depths_by_session"
        if no_ext_path.exists():
            m1_depths_file = no_ext_path
        else:
            raise FileNotFoundError(
                f"Missing M1 depths file in '{spiking_data_dir}' "
                "(expected m1_cell_depths_by_session.json)."
            )
    ...
    spikes_by_cell = defaultdict(list)
    with open(thalamic_spikes_file, "r", newline="") as fileObj:
        reader = csv.DictReader(fileObj)
        if reader.fieldnames is None or "spkid" not in reader.fieldnames or "spkt" not in reader.fieldnames:
            raise ValueError(
                f"Unexpected thalamic spikes CSV schema in '{thalamic_spikes_file}'. "
                "Expected columns: spkid, spkt."
            )
        for row in reader:
            raw_id = row.get("spkid")
            raw_time = row.get("spkt")
            if raw_id in (None, "") or raw_time in (None, ""):
                continue
            cell_id = int(float(raw_id))
            spike_time = float(raw_time)
            spikes_by_cell[cell_id].append(spike_time)

    if not spikes_by_cell:
        raise ValueError(f"No spikes found in '{thalamic_spikes_file}'.")

    for cell_id in sorted(spikes_by_cell.keys()):
        spikeTimesList.append(sorted(spikes_by_cell[cell_id]))

    with open(m1_depths_file, "r") as fileObj:
        m1_depths_by_session = json.loads(fileObj.read())

    if not isinstance(m1_depths_by_session, dict):
        raise ValueError(
            f"Unexpected M1 depths format in '{m1_depths_file}'. Expected a dict by session."
        )

    for folder, cellDepths in m1_depths_by_session.items():
        counts = cellPerlayer(cellDepths)
        M1sampledCells.append(counts)
        foldersName.append(folder)

    baseline_period = getattr(cfg, "thalBaselinePeriod", baselinePeriod)
    baselineSpks, movementAndPostSpks = SampleSpikes(
        spikeTimesList, cfg, baselinePeriod=baseline_period, skipEmpty=skipEmpty
    )
