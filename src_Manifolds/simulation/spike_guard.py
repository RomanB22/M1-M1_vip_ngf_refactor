...
def record_local_spike_guard(sim: Any, cfg: Any) -> None:
    """Attach per-rank guard metrics before ``sim.gatherData()``."""

    if cfg.spikeGuard.get("enabled", False):
        sim.simData["spikeGuardLocal"] = collect_local_guard_metrics(
            sim.net.cells, cfg.spikeGuard
        )


def summarize_simulation_spike_guard(sim: Any, cfg: Any) -> dict[str, Any] | None:
    """Build the rank-zero guard summary after gathered simulation data exist."""

    if not cfg.spikeGuard.get("enabled", False):
        return None
    return summarize_guard_metrics(
        sim.allSimData.get("spikeGuardLocal", {}),
        cfg.spikeGuard,
        bool(cfg.singleCellPops and cfg.testCellsPerPop == 1),
    )
