...
from cfg import cfg
from simulation import network_helpers
from simulation.runtime_config import PROJECT_ROOT as REPOSITORY_ROOT, prepare_runtime_config
...
netParams.correctBorder = {'threshold': [cfg.correctBorderThreshold, cfg.correctBorderThreshold, cfg.correctBorderThreshold], 
                        'yborders': [cfg.layer['2'][0], cfg.layer['5A'][0], cfg.layer['6'][0], cfg.layer['6'][1]]}  # correct conn border effect
...
from simulation.spike_guard import inject_guard_into_netparams, install_netpyne_spike_source_patch
...
def _module_dir() -> Path:
    ...
    # When executed via importlib loaders without __file__
    spec = g.get("__spec__")
    if spec is not None and getattr(spec, "origin", None):
        return Path(spec.origin).resolve().parent
    # Fallback (rare): use inspect; last resort: CWD
    try:
        return Path(inspect.getsourcefile(lambda: None)).resolve().parent  # type: ignore[arg-type]
    except Exception:
        return Path.cwd()
...
def parse_cfg_mutations(cfg):
    out = []
    for m in getattr(cfg, "mutations", []):
...
if cfg.diversity:
    low = 1.0 - cfg.pasUniformFrac
    high = 1.0 + cfg.pasUniformFrac
...
if cfg.addLongConn:
    connLongData = network_helpers.addLongConnections(cwd, netParams, cfg)

if cfg.singleCellPops:
    # A few cells per population keep the smoke model cheap while providing
    # enough layer-matched features for the configured manifold reference.
    # Seven is the current default because L5B needs 39 features and contains
    # six cortical populations (6 * 7 >= 39).
    for pop in netParams.popParams.values():
        pop['numCells'] = int(cfg.testCellsPerPop)
...
netParams.synMechParams['GABAA'] = {'mod':'MyExp2SynBB', 'tau1': 0.07, 'tau2': 18.2, 'e': 'uniform(-80, -70)'}
netParams.synMechParams['GABAA_VIP'] = {'mod':'MyExp2SynBB', 'tau1': 0.3, 'tau2': 6.4, 'e': 'uniform(-80, -70)'}  # Pi et al 2013
netParams.synMechParams['GABAASlow'] = {'mod': 'MyExp2SynBB','tau1': 2, 'tau2': 100, 'e': 'uniform(-80, -70)'}
...
if cfg.addSubConn:
    ...
    for k in synDens.keys():
        ...
        postyRange = list(cfg.layer[post.split('L')[1]]) # get layer yfrac range 
        if post == 'L2': postyRange[1] = cfg.layer['4'][1]  # apply L2 rule also to L4 
        netParams.subConnParams[k] = {
        'preConds': {'pop': prePop}, 
...
report = network_helpers.filter_by_enabled_cells_yaml(
    netParams,
